...
export const SignUpBox = () => {
  ...
  const signUpHandler = async (fullName, userName, email, password, loginHandler) => {
      try{
  ...
  return (
    <div className="signup-box">
        ...
        type="text" onChange={(event) => setFullName(event.target.value)} />
      ...
      className="signup-item nm-btn2 an"
        onClick={() => signUpHandler(fullName, userName, email, password, loginHandler)}
      >
        Sign Up
      </button>
    </div>
